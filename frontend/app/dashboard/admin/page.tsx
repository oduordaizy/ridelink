                    ...
                    {statCards.map((card, i) => (
                        <div key={i} className="bg-white p-4 sm:p-6 md:p-8 rounded-2xl md:rounded-3xl shadow-sm border border-gray-100 hover:shadow-lg hover:scale-105 transform transition-all duration-300">
                            <div className="flex items-center justify-between mb-3 sm:mb-4">
                                ...
                                <div className={`flex items-center space-x-1 text-sm sm:text-base font-bold ${card.trendUp ? 'text-green-500' : 'text-red-500'}`}>
                                    {card.trendUp ? <IoTrendingUp /> : <IoTrendingDown />}
                                    <span>{card.trend}</span>
                                </div>
                            </div>
                            <h3 className="text-gray-500 text-sm sm:text-base font-medium">{card.title}</h3>
                            <p className="text-xl sm:text-2xl font-bold text-[#00204a] mt-1 break-words">{card.value}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-50 rounded-xl">
                                <div className="flex items-center space-x-4 mb-2 sm:mb-0">
                                    ...
                                    <div>
                                        <p className="text-sm sm:text-base font-bold text-[#00204a]">{stats?.users.new_today} New Registrations</p>
                                        <p className="text-xs text-gray-400">Since 12:00 AM today</p>
                                    </div>
                                </div>
                                <button className="text-xs sm:text-sm font-bold text-[#08A6F6] hover:underline">View All</button>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-gray-50 rounded-xl">
                                <div className="flex items-center space-x-4 mb-2 sm:mb-0">
                                    ...
                                    <div>
                                        <p className="text-sm sm:text-base font-bold text-[#00204a]">{stats?.payments.failed} Failed Payments</p>
                                        <p className="text-xs text-gray-400">Needs investigation</p>
                                    </div>
                                </div>
                                <button className="text-xs sm:text-sm font-bold text-red-500 hover:underline">Review Now</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#00204a] p-8 rounded-2xl shadow-sm text-white">
                        ...
                        <div className="space-y-3">
                            <button className="w-full py-3 sm:py-4 bg-white/10 hover:bg-white/20 rounded-2xl text-base sm:text-sm font-medium transition-colors text-left px-4 flex items-center space-x-3">
