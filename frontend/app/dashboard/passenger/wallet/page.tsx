  ...
  const stkPushQueryWithIntervals = (CheckoutRequestID: string) => {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          // If we get an error from M-Pesa, we might still want to poll unless it's a fatal error
          return;
        }
        ...
        if (data.ResultCode === '0') {
          ...
          setTimeout(() => {
            // We can't easily call fetchWalletData from here unless we move it
            window.location.reload();
          }, 3000);
        } else if (data.ResultCode) {
          // Any other ResultCode means failure
          clearInterval(timer);
          setStkQueryLoading(false);
          setIsProcessing(false);
