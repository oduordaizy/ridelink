...
export async function POST(request: Request) {
    try {
        ...
        if (!isSafaricomIp(ip)) {
            console.warn(`Blocked unauthorized M-Pesa result callback from IP: ${ip}`);
        ...
        if (!requestSecret || requestSecret !== expectedSecret) {
            console.warn(`Blocked M-Pesa result callback with invalid secret from IP: ${ip}`);
        ...
        if (!isValidMpesaPayload(body)) {
            console.warn(`Blocked malformed M-Pesa result payload`);
            return NextResponse.json({ ResultCode: 1, ResultDesc: "Malformed Payload" }, { status: 400 });
        }

        console.log('Received valid M-Pesa result callback:', JSON.stringify(body));
        ...
        // Fire and forget, but await so we can log errors if the backend is unreachable
        ...
        }).catch(err => console.error('Error forwarding M-Pesa result to Django:', err));
